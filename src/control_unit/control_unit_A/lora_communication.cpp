...
int restartInertialUnit(double azimuth, int calibrationMatrix[4][4]) {
  ...
  if (flagSend){
    ...
    if(azimuth!=-1 || calibrationMatrix==0 && azimuth==-1){
      Serial.println(azimuth);
      restartCommand = "RESTART_INERTIAL_UNIT:";
      restartCommand += String(azimuth, 2);  // Add azimuth with 2 decimal places
    }
    else{
      restartCommand = "|SET_CALIBRATION_MATRIX:";
      for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                restartCommand += String(calibrationMatrix[i][j]);
                if (!(i == 3 && j == 3)) {
                    restartCommand += ",";  // Čiarky medzi hodnotami
                }
            }
        }
    }
